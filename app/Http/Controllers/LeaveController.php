...
class LeaveController extends Controller {
	...
	public function show($id)
	{
		if (request()->ajax())
		{
			...
			return response()->json(['data' => $data, 'employee_name' => $employee_name, 'organisatie' => $organisatie, 'department' => $department, 'leave_type_name' => $leave_type_name,
	...
	public function delete_by_selection(Request $request)
	{
		...
		if ($logged_user->can('delete-leave'))
		{
			...
			if ($leave->delete())
			{
				return response()->json(['success' => __('Multi Delete', ['key' => trans('Leave')])]);
			} else
			{
	...
	public function calendarableDetails($id)
	{
		if (request()->ajax())
		{
			$data = Leave::with('company:id,organisatie',
