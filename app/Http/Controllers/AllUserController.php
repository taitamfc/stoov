...
class AllUserController extends Controller {
                ...
                if (request()->ajax()){
                    ...
                    ->setRowId(function ($user)
                        ...
                        ->addColumn('username', function ($row)
                        {
                            ...
                            $username = "<span><b>Gebruikersnaam:</b> &nbsp;".$row->username."</span>";
                        ...
                        {
                            return '<b>Laatste aanmeldingsdatum :</b> '.$row->last_login_date.'</br>'.'<b>Laatste Login IP :</b> '.$row->last_login_ip;
                        })
	...
	public function delete_by_selection(Request $request)
	{
		...
		if ($logged_user->can('delete-user'))
		{
			...
			$user = User::whereIn('id', $user_id)->where('role_users_id','!=',1);

			if ($user->delete())
			{
				return response()->json(['success' => __('Multi Delete', ['key' => trans('User')])]);
			} else
			{
