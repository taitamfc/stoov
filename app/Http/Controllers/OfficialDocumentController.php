...
class OfficialDocumentController extends Controller {
	...
	public function index()
	{
		...
		if ($logged_user->can('view-official_document'))
		{
			if (request()->ajax())
			{
				$official_documents = OfficialDocument::with('company:id,organisatie', 'AddedBy:id,username', 'DocumentType:id,document_type')
					->get();
				...
				return datatables()->of($official_documents)
					...
					{
						return $row->company->organisatie ?? ' ' ;
					})
					...
					->addColumn('title', function ($row)
					{
						...
						{
							return $row->document_title . $row->identificaton_number . '<br><h6><a href="' . route('official_documents.downloadFile', $row->id) . '">' . trans('File') . '</a></h6>';
						} else
						{
					...
					{
						$button = '<h4><a href="' . route('official_documents.downloadFile', $data->id) . '">' . trans('Download') . '</a></h4>';
	...
	public function delete_by_selection(Request $request)
	{
		...
		if ($logged_user->can('delete-official_document'))
		{
			...
			return response()->json(['success' => __('Multi Delete', ['key' => trans('File')])]);
		}
		return response()->json(['success' => __('You are not authorized')]);
	}
