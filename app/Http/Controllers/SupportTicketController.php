...
class SupportTicketController extends Controller {
	...
	public function index()
	{
		...
		if ($logged_user->can('view-ticket'))
		{
			if (request()->ajax())
			{
				return datatables()->of(SupportTicket::with('company:id,organisatie', 'employee:id,first_name,last_name', 'department:id,department_name')->get())
					...
					->addColumn('company', function ($row)
					{
						...
						return $organisatie . ' (' . $department_name . ')';
					})
					...
					->addColumn('ticket_details', function ($row)
					{
						...
						{
							return $row->ticket_code . '<br><h6><a href="' . route('tickets.downloadTicket', $row->id) . '">' . trans('File') . '</a></h6>';
						} else
						{
