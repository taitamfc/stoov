...
//				]
	...
	public function show($id)
	{
		if (request()->ajax())
		{
			...
			return response()->json(['data' => $data, 'organisatie' => $organisatie, 'department' => $department,
...
//				]
	...
	public function delete_by_selection(Request $request)
	{
		...
		if ($logged_user->can('delete-event'))
		{
			...
			if ($event->delete())
			{
				return response()->json(['success' => __('Multi Delete', ['key' => trans('Event')])]);
			} else
			{
	...
	public function calendarableDetails($id)
	{
		if (request()->ajax())
		{
			$data = Event::with('company:id,organisatie',
				'department:id,department_name')->findOrFail($id);

			$new = [];

			$new['Company Name'] = $data->company->organisatie;
