		...
		if ($logged_user->can('view-expense'))
		{
			if (request()->ajax())
			{
				return datatables()->of(FinanceExpense::with('Account', 'PaymentMethod', 'Payee', 'Category')->latest('expense_date'))
					...
					{
						return $row->PaymentMethod->method_name ?? trans('Bank');
					})
					...
					->addColumn('action', function ($data)
					{
						...
						$button .= '&nbsp;&nbsp;';
						if (auth()->user()->can('edit-expense') && $data->expense_reference !== trans('Payroll'))
						{

							$button .= '<button type="button" name="edit" id="' . $data->id . '" class="edit btn btn-primary btn-sm"><i class="dripicons-pencil"></i></button>';
							$button .= '&nbsp;&nbsp;';
						}
						if (auth()->user()->can('delete-expense') && $data->expense_reference !== trans('Payroll'))
						{
