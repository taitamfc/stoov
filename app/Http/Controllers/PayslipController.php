			...
			if (request()->ajax())
			{
				...
				return datatables()->of($payslips)
					...
					->addColumn('employee_name', function ($row)
					{
						...
						$organisatie = $row->employee->company->organisatie ?? '';
						$department_name = $row->employee->department->department_name ?? '';

						return $employee_name.'</br><b>Comapny :</b> '.$organisatie.'</br><b>Department :</b> '.$department_name;
					})
					->addColumn('salary_details', function ($row)
					{
						...
						if($row->payment_type == 'Hourly') { //Basic Salary (Total)
							$employee = Employee::with('user:id,username','company:id,organisatie','department:id,department_name','designation:id,designation_name')
